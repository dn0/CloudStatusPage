...
module "vm_mon-agent_ap-northeast-2" {
  source    = "../modules/vm_mon-agent"
  providers = { aws = aws.ap-northeast-2 }

  vpc_subnet_ids      = module.config_ap-northeast-2.vpc_subnet_ids
  ...
  env = merge(local.env, {
    AWS_EC2_VM_VPC_ID = module.config_ap-northeast-2.vpc_id,
  })

  depends_on = [module.config_ap-northeast-2]
}
