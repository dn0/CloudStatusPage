...
module "config_ca-west-1" {
  source    = "../modules/config_region"
  providers = { aws = aws.ca-west-1 }
  account   = local.account
}

module "vm_mon-agent_ca-west-1" {
  source    = "../modules/vm_mon-agent"
  providers = { aws = aws.ca-west-1 }

  vpc_subnet_ids      = module.config_ca-west-1.vpc_subnet_ids
  ...
  env = merge(local.env, {
    AWS_EC2_VM_VPC_ID = module.config_ca-west-1.vpc_id,
  })

  depends_on = [module.config_ca-west-1]
}
